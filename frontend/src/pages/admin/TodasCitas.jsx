...
import { obtenerTodasLasCitas, eliminarCita, limpiarCitasCanceladas } from '../../services/adminApi';
...
export default function TodasCitas() {
  ...
  const [dialogLimpiar, setDialogLimpiar] = useState(false);
  ...
  const handleLimpiarCitasCanceladas = async () => {
    try {
      setLimpiarLoading(true);
      const result = await limpiarCitasCanceladas();
      setSuccess(`${result.total_eliminadas} citas canceladas eliminadas exitosamente`);
      setDialogLimpiar(false);
      cargarCitas();
      setTimeout(() => setSuccess(null), 3000);
    } catch (err) {
      console.error('Error al limpiar citas:', err);
      setError('Error al limpiar citas canceladas');
    } finally {
    ...
    <Box>
      ...
      <Box sx={{ mb: 3, display: 'flex', gap: 2 }}>
        <Button
          ...
          onClick={() => setDialogLimpiar(true)}
        >
          Limpiar citas canceladas
        </Button>
      </Box>
      ...
      {/* Dialog de confirmación para eliminar una cita */}
      ...
      {/* Dialog de confirmación para limpiar todas las citas canceladas */}
      <Dialog open={dialogLimpiar} onClose={() => setDialogLimpiar(false)}>
        <DialogTitle>⚠️ Limpiar citas canceladas</DialogTitle>
        <DialogContent>
          <Typography>
            ¿Estás seguro de que deseas eliminar TODAS las citas con estado cancelado? 
          </Typography>
        ...
        <DialogActions>
          <Button onClick={() => setDialogLimpiar(false)} color="inherit">
            Cancelar
          </Button>
          <Button 
            onClick={handleLimpiarCitasCanceladas} 
            color="error" 
            variant="contained"
            disabled={limpiarLoading}
          >
            {limpiarLoading ? 'Limpiando...' : 'Limpiar todo'}
          </Button>
        </DialogActions>
      </Dialog>
    </Box>
